...
function Notifications() {
  ...
  return (
    <div className={`min-h-screen pb-20 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      <div className={`fixed top-0 left-0 right-0 border-b px-4 py-4 z-50 ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
        <h1 className={`text-lg font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Notifications</h1>
      </div>
      ...
      <div className="pt-16">
        {loading ? (
          ...
          notifications.map((notif) => (
            <div key={notif.id} className={`flex items-center gap-3 px-4 py-4 border-b ${
              !notif.is_read
                ? darkMode ? 'bg-gray-800 border-gray-700' : 'bg-[#8EB3E7]/10 border-gray-100'
